...
DecodeResult* OvcDecoder::Decode(const uint8_t* InNalBytes, size_t InNalSize)
{
	...
	if (Decoder->get_picture(&DecodedPicture) == OVC_DEC_OK)
	{
		if (OnDecodedImageCallback != nullptr)
		{
			...
			Image.Bytes.resize(DecodedPicture.info.width * DecodedPicture.info.height);
			memcpy(Image.Bytes.data(), DecodedPicture.Y, DecodedPicture.info.width * DecodedPicture.info.height);
			Image.Size = DecodedPicture.info.width * DecodedPicture.info.height;
